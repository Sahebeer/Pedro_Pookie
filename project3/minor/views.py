...
from .forms import UserInfoForm, FacultyInfoForm, GroupInfoForm, ProjectForm, ProjectApprovalByMentorForm, \
    ProjectApprovalByHODForm
...
def coordinator_home(request):
    ...
    if coordinator:
        if GroupInfo.objects.filter(department=faculty.department, section=coordinator.section,
                                      year=coordinator.year):
            group = GroupInfo.objects.filter(department=faculty.department, section=coordinator.section,
                                             year=coordinator.year)
    my_project = []
    if group:
        for i in group:
